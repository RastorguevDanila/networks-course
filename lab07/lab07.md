...
1. Какой IP-адрес и номер порта TCP использует ваш компьютер (отправитель), передающий
   ...
   - Source Address: 192.168.0.104, Source Port: 55190 (картинка 1.1)
2. Каков IP-адрес у сервера gaia.cs.umass.edu? Каковы номера портов для отправки и приема
   ...
   - Destination Port: 80, при отправке будет тот же порт. (1.2)
3. Какой порядковый номер у SYN TCP-сегмента, который используется для установления
   ...
   - 274. Раньше нет SYN-сегмента от моего компьютера до сервера. Я проверил. 1.3
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   - 275. 
   - 1
   - 1, определил значение поля подтверждения (1), взяв порядковый номер из SYN-сегмента, полученного от клиента (пакет №274, его Sequence number был 0), и прибавив к нему 1. 
   - Есть флаги SYN, ACK 1.4
...
#### Скрин
done. скрин называется 2. Ещё есть ласт задача.
...
#### Решение
Чтобы канал был загружен на U процентов, размер окна в битах (W_bits) должен быть таким, чтобы за время RTT передавалось W_bits данных, и это составляло U от максимальной возможности канала за это же время.
Пропускная способность = Размер окна (в битах) / RTT
Использование канала U = Пропускная способность / Скорость канала R
U = (W_bits / RTT) / R
U = W_bits / (R * RTT)
Отсюда W_bits = U*R*RTT = 0.98*10^9*0.03 = 29400000 бит. Размер пакета равен 12000 бит. n=W_bits/пакет =29400000/12000=2450. Поскольку нам надо строго больше 98%, а деление получилось целым, придётся добавить ещё 1. иттого 2451.
