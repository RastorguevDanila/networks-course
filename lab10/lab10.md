...
1. Выберите первое ICMP-сообщение эхо-запроса, отправленное вашим компьютером, и
   ...
   - Source Address: 192.168.0.104. Скрин pp1.
...
3. Сколько байт в IP-заголовке? Сколько байт приходится на полезную нагрузку IP-дейтаграммы?
   - .... 0101 = Header Length: 20 bytes (5). pp1
   - 56 байт - это данные ICMP, 8 байт - заголовок ICMP, итого полезная нагрузка - 64 байта.
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   1. Какие поля IP-дейтаграммы всегда изменяются от одной дейтаграммы к
      ...
      - Time to live, identification, header checksum (можно сравнить с pp1 на котором это видно).
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - Version, header length, source ip adress, destination ip adress, protocol - не меняются и не должны
      - должны изменяться все из пункта 1, они и меняются. Скрин pp4.
...
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - TTL не изменился, а вот Identificatiion почему-то и там и там 0. (pp6.1, pp6.2)
...
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами? 
      ...
      - Да было т.к. тут и fragment offset=0, и флаг (pp8). Фрагментов - 3, они в заросах 92 93 94.
   2. Какие поля IP-заголовка изменяются в разных фрагментах?
      - Total length, у 92 и 93 он 1500 (max), у 94 -- 568. Также очевидно похожая история с флагом "More fragments", которого не будет в 94. fragment offset -- 0, 1480, 2960; header checksum. Для наглядности скрин pp8.2
