...
#### Решение
1. Для начала найдём общее число отправленных пакетов. Это N = W/2 + (W/2+1) + ... + W - сумма арифм. прогрессии. Она равна (W+W/2)(W/2+1)/2 = 1/8*(3W * (W+2)) = 3/8 W^2 + 3/4W. L = 1/N - чтд.
2. Средний размер окна равен 3/4 * W, средняя скорость равна 3/4*W * MSS/RTT. 
Теперь обработаем то, что написано. 1.22 приблизительно равно sqrt(6)/2, подставим вместо L то что мы доказали в 1 пункте. Получим sqrt(6)*MSS/(2RTT) * sqrt((3/8)W^2+3/4*W). Линейной частью можно пренебречь, ибо W  обычно большое. остаётся sqrt(6*3/8)MSS*W/(2RTT) = MSS*W/RTT *sqrt(9/16) = 3/4MSS * W/RTT - средняя скорость.
...
#### Решение
1. Не вижу концептуального отличия от 3 задачи из прошлой дз, в ней получилось L = 2a/(W(1+2a)), но это a -- другое, ведь увеличение было в (1+a) раз, а не в a. Ну давайте выразим, пусть x=(a+1) (то есть выразим не через a, а через (1+a)). получим L=(2x-2)/(W(2x-1)). Теперь поймём что x - это наше новое а (опять же, потому что мы увеличивали в (1+a) раз, а теперь просто в a). Ну то есть ответ L=(2a-2)/(W(2a-1)). Наверное проще было перерешать по-человечески.
2. нам потребуется log_a(2) RTT-циклов для расширения окна, то есть log_a(2)*RTT c., что не зависит от средней пропускной способности, т.к. не зависит от W, L (от которых зависит средняя пропускная способность).