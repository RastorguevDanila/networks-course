...
#### Решение
Как я понял, измерять в RTT = измерять в количестве циклов RTT, а для этого нам надо бы знать T_{RTT} - длительность RTT цикла в секундах.
Средний размер окна равен 3W/4, в байтах - MSSS*3w/4 (по определению MSSS). Тогда S = MSSS*(3W/4)/T_{RTT} - средняя пропускная способность (в байт/с). С другой стороны, T=(W-W/2)/1 = W/2, то есть W=2T. Подставим и получим S=MSSS*T*3/(2T_{RTT}), откуда T=2/3*(ST_{RTT}/(MSSS))
...
#### Решение
Общее время T = (Время установления соединения) + (Время для окна 1S и его ACK) + (Время для окна 2S и его ACK) + (Время для окна 4S и его ACK) + (Время передачи данных окна 8S) = RTT + max(S/R, RTT) + max(2S/R, RTT) + max(4S/R, RTT)+8S/R.
1. RTT + RTT + max(2S/R, RTT) + 4S/R + 8S/R = max(2S/R, RTT) + 2RTT + 12S/R
2. RTT + RTT + RTT + \max(4S/R, RTT) + 8S/R$ = 3RTT + \max(4S/R, RTT) + 8S/R 
3. RTT + S/R + 2S/R + 4S/R +8S/R = RTT+15S/R
...
#### Решение
Найдём количество переданных сегментов. Для этого за k обозначим log_{1+a}(2) - именнш столько увеличений нам потребуется. 
W/2(1+(1+a)+(1+a)^2+...+(1+a)^k) = W/2*((1+a)^(k+1)-1)/a =W/2*(2a+2-1)/a = W(1+2a)/2a. => L = 2a/(W(1+2a)).
...
#### Решение
1. Соединение Клиент -> FE. 1.5*RTT_{FE} (1 рукопожатие + 0.5 доставка запроса на FE)
2. Взаимодействие FE и BE. 
Пересылаем FE->BE: 1/2 RTT_{BE}
Обрабатываем в датацентре за T_{обработки}
Пересылаем ответ за 1/2 RTT_{BE}
итог: +RTT_{BE}+T_{обработки}
3. FE -> клиент: 
1 окно клиенту за 0.5 RTT_{FE}
Отправка подтвержждения от клиента + отправка 2 окна за RTT_{FE}
то же для 3 окна, +RTT_{FE}
Подтверждение для 3 окна уже не влияет на измеряемое время получения ответа клиентом, т.к. раньше мы ждали подтвержденияя для пересылки следующего окна, а больше пересылать нечего.
итог: +2.5*RTT_{FE}
Суммируем и получаем требуемое